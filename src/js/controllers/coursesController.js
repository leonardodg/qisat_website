						...
						if (vm.search) {
							...
							if (item.info && item.info.conteudo) {
								for (i = 0; i < item.info.conteudo.length; i++) {
									...
									for (k = 0; k < search.length; k++) {
										s = search[k];
										if (!((j.titulo && retira_acentos(j.titulo).toLowerCase().indexOf(retira_acentos(s).toLowerCase()) >= 0) || (j.descricao && retira_acentos(j.descricao).toLowerCase().indexOf(retira_acentos(s).toLowerCase()) >= 0))) {
											retorno = false;
											break;
										}
									}
							...
							if (item.eventos) {
								for (i = 0; i < item.eventos.length; i++) {
									...
									for (k = 0; k < search.length; k++) {
										s = search[k];
										if (!((j.cidade && j.cidade.nome && retira_acentos(j.cidade.nome).toLowerCase().indexOf(retira_acentos(s).toLowerCase()) >= 0) || (j.cidade && j.cidade.estado && j.cidade.estado.nome && retira_acentos(j.cidade.estado.nome).toLowerCase().indexOf(retira_acentos(s).toLowerCase()) >= 0))) {
											retorno = false;
											break;
										}
									}
							...
							for (k = 0; k < search.length; k++) {
								s = search[k];
								if (!((item.nome && retira_acentos(item.nome).toLowerCase().indexOf(retira_acentos(s).toLowerCase()) >= 0) || (item.info && item.info.metatag_key && retira_acentos(item.info.metatag_key).toLowerCase().indexOf(retira_acentos(s).toLowerCase()) >= 0))) {
									retorno = false;
									break;
								}
							}
